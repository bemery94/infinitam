...
void UIEngine::glutDisplayFunction() {
  ...
  uiEngine->mainEngine->GetImage(
      uiEngine->outImage[0], uiEngine->outImageType[0], &uiEngine->freeviewPose,
      &uiEngine->freeviewIntrinsics);

  for (int w = 1; w < NUM_WIN; ++w)
    uiEngine->mainEngine->GetImage(uiEngine->outImage[w],
                                   uiEngine->outImageType[w]);
  ...
  glPushMatrix();
  {
    ...
    glPushMatrix();
    {
      ...
      for (int w = 0; w < NUM_WIN; ++w) {  // Draw each sub window
        if (uiEngine->outImageType[w] == ITMMainEngine::InfiniTAM_IMAGE_UNKNOWN)
          continue;
        glBindTexture(GL_TEXTURE_2D, uiEngine->textureId[w]);
        glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, showImgs[w]->noDims.x,
                     showImgs[w]->noDims.y, 0, GL_RGBA, GL_UNSIGNED_BYTE,
  ...
  if (uiEngine->freeviewActive) {
    sprintf(str,
            "n - next frame \t b - all frames \t e/esc - exit \t f - "
            "follow camera \t c - colours (currently %s) \t t - turn "
            "fusion %s",
            uiEngine->colourModes[uiEngine->currentColourMode].name,
            uiEngine->intergrationActive ? "off" : "on");
  } else {
    sprintf(str,
            "n - next frame \t b - all frames \t e/esc - exit \t f - free "
            "viewpoint \t t - turn fusion %s",
            uiEngine->intergrationActive ? "off" : "on");
  }
...
void UIEngine::glutKeyUpFunction(unsigned char key, int x, int y) {
  ...
  switch (key) {
    ...
    case 'f':
      if (uiEngine->freeviewActive) {
        ...
        uiEngine->freeviewPose.SetFrom(
            uiEngine->mainEngine->GetTrackingState()->pose_d);
        if (uiEngine->mainEngine->GetView() != NULL) {
          uiEngine->freeviewIntrinsics =
              uiEngine->mainEngine->GetView()->calib->intrinsics_d;
          uiEngine->outImage[0]->ChangeDims(
              uiEngine->mainEngine->GetView()->depth->noDims);
        }
    ...
    case 'c':
      ...
      if ((unsigned)uiEngine->currentColourMode >=
          uiEngine->colourModes.size()) {
  ...
  if (uiEngine->freeviewActive) {
    uiEngine->outImageType[0] =
        uiEngine->colourModes[uiEngine->currentColourMode].type;
  }
}
...
static inline Matrix3f createRotation(const Vector3f& _axis, float angle) {
  ...
  ret *= co;
  for (int r = 0; r < 3; ++r)
    for (int c = 0; c < 3; ++c) ret.at(c, r) += (1.0f - co) * axis[c] * axis[r];
...
void UIEngine::glutMouseMoveFunction(int x, int y) {
  ...
  switch (uiEngine->mouseState) {
    case 1: {
      ...
      float angle =
          scale_rotation *
          sqrt((float)(movement.x * movement.x + movement.y * movement.y));
      Matrix3f rot = createRotation(axis, angle);
      uiEngine->freeviewPose.SetRT(rot * uiEngine->freeviewPose.GetR(),
                                   rot * uiEngine->freeviewPose.GetT());
    ...
    case 2: {
      ...
      uiEngine->freeviewPose.SetT(
          uiEngine->freeviewPose.GetT() +
          scale_translation *
              Vector3f((float)movement.x, (float)movement.y, 0.0f));
      uiEngine->needsRefresh = true;
      break;
    }
    case 3: {
      ...
      uiEngine->freeviewPose.SetT(uiEngine->freeviewPose.GetT() +
                                  scale_translation *
                                      Vector3f(0.0f, 0.0f, (float)movement.y));
      uiEngine->needsRefresh = true;
      break;
    }
    default:
...
void UIEngine::Initialise(int& argc, char** argv,
                          ...
                          IMUSourceEngine* imuSource, ITMMainEngine* mainEngine,
                          const char* outFolder,
                          ITMLibSettings::DeviceType deviceType) {
  CHECK_NOTNULL(imageSource);
  CHECK_NOTNULL(mainEngine);
  CHECK_NOTNULL(outFolder);
  this->freeviewActive = false;
  this->intergrationActive = true;
  this->currentColourMode = 0;
  this->colourModes.push_back(UIColourMode(
      "shaded greyscale", ITMMainEngine::InfiniTAM_IMAGE_FREECAMERA_SHADED));
  ...
  // winSize.x = 2 * MAX(imageSource->getRGBImageSize().x,
  // imageSource->getDepthImageSize().x);
  // winSize.y = MAX(imageSource->getRGBImageSize().y,
  // imageSource->getDepthImageSize().y) + textHeight;
  ...
  int textHeight = 30;  // Height of text area
  // winSize.x = (int)(1.5f * (float)MAX(imageSource->getImageSize().x,
  // imageSource->getDepthImageSize().x));
  // winSize.y = MAX(imageSource->getRGBImageSize().y,
  // imageSource->getDepthImageSize().y) + textHeight;
  ...
  for (int w = 0; w < NUM_WIN; ++w)
    outImage[w] =
        new ITMUChar4Image(imageSource->getDepthImageSize(), true, allocateGPU);
...
void UIEngine::GetScreenshot(ITMUChar4Image* dest) const {
  glReadPixels(0, 0, dest->noDims.x, dest->noDims.y, GL_RGBA, GL_UNSIGNED_BYTE,
...
void UIEngine::ProcessFrame() {
  ...
  // std::cout << "gettingImages" << std::endl;
